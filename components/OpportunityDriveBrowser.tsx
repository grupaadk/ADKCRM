...
export default function OpportunityDriveBrowser({
  opportunityId,
  rootFolderId,
  onFolderChange,
  refreshKey,
  ...
  const createFolder = useAction(api.googleDrive.createOpportunityFolderInDrive);
  ...
  const [breadcrumb, setBreadcrumb] = useState<BreadcrumbEntry[]>(
    rootFolderId ? [{ id: rootFolderId, name: "Folder szansy" }] : [],
  );
  ...
  if (rootFolderId !== prevRootFolderId) {
    ...
    setBreadcrumb(rootFolderId ? [{ id: rootFolderId, name: "Folder szansy" }] : []);
    setItems([]);
  }

  const currentFolderId = breadcrumb[breadcrumb.length - 1]?.id;
  const currentFolderName = breadcrumb[breadcrumb.length - 1]?.name ?? "Folder szansy";
  ...
  useEffect(() => {
    ...
    const fetchContents = async () => {
      ...
      setFetchError(null);
      try {
        const result = await listFolderContents({ opportunityId, folderId: currentFolderId });
        if (!cancelled) {
          setItems(result);
          setLoading(false);
        }
  ...
  }, [currentFolderId, opportunityId, refreshCounter, refreshKey, rootFolderId, listFolderContents]);
  ...
  return (
    <section className="panel" style={{ overflow: "hidden" }}>
      ...
      >
        <span className="up mute">Pliki szansy sprzedaży</span>
      ...
      <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
        {!rootFolderId && (
          ...
          >
            Folder Google Drive nie istnieje dla tej szansy sprzedaży.
          </p>
        )}
