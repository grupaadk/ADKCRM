...
export default function OrderDetailPage({
  ...
  const [sentFilePreviewModal, setSentFilePreviewModal] = useState<{
    name: string;
    url: string;
  ...
  useEffect(() => {
    if (!sentFilePreviewModal) return;
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") setSentFilePreviewModal(null);
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [sentFilePreviewModal]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                              ...
                              return (
                                <div key={fIdx} className="flex items-center justify-between p-2 bg-white rounded border border-emerald-200 text-xs shadow-xs hover:border-emerald-300 transition-colors">
                                  ...
                                  <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                                    ...
                                    {/* Dwie akcje: 👁️ Podgląd oraz 📁 Google Drive */}
                                    <div className="flex items-center gap-1 border-l border-slate-200 pl-2 ml-1">
                                      <button
                                        type="button"
                                        onClick={() => {
                                          if (previewUrl) {
                                            setSentFilePreviewModal({
                                              name: file.fileName,
                                              url: previewUrl,
                                              driveUrl: driveUrl,
                                            });
                                          } else {
                                            alert("Brak dostępnego podglądu pliku.");
                                          }
                                        }}
                                        ...
                                        disabled={!previewUrl}
                                        title={previewUrl ? "Podgląd pliku w aplikacji" : "Brak podglądu dla tego pliku"}
                                      >
                                      ...
                                      {driveUrl ? (
                                        <a
                                          ...
                                          title="Otwórz plik w Google Drive"
                                        >
                                        ...
                                        <button
                                          ...
                                          title="Plik nie posiada identyfikatora z Google Drive"
                                        >
                                          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                            <path d="M19.5 21a3 3 0 003-3v-4.5a3 3 0 00-3-3h-15a3 3 0 00-3 3V18a3 3 0 003 3h15zM1.5 10.146V6a3 3 0 013-3h5.379a2.25 2.25 0 011.59.659l2.122 2.121c.14.141.331.22.53.22H19.5a3 3 0 013 3v1.146A4.483 4.483 0 0019.5 9h-15a4.483 4.483 0 00-3 1.146z" />
                                          </svg>
      ...
      {/* Modal Podglądu Pliku Wysłanego po API */}
      {sentFilePreviewModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs"
          onClick={() => setSentFilePreviewModal(null)}
        >
          <div
            className="relative w-full max-w-5xl h-[85vh] rounded-2xl bg-white shadow-2xl ring-1 ring-black/10 overflow-hidden flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header Bar */}
            <div className="flex items-center justify-between px-5 py-3.5 bg-slate-900 text-white flex-shrink-0">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <span className="font-semibold text-sm truncate" title={sentFilePreviewModal.name}>
                  Podgląd: {sentFilePreviewModal.name}
                </span>
              </div>

              <div className="flex items-center gap-2 flex-shrink-0">
                {sentFilePreviewModal.driveUrl ? (
                  <a
                    href={sentFilePreviewModal.driveUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-emerald-300 bg-emerald-950/60 hover:bg-emerald-900 border border-emerald-700/60 rounded-lg transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  ...
                  <span className="text-xs text-slate-400 italic px-2">Brak pliku w Google Drive</span>
                )}
            ...
            <div className="flex-1 bg-slate-950 relative overflow-hidden flex items-center justify-center">
              {(() => {
                let iframeSrc = sentFilePreviewModal.url;
                if (iframeSrc.includes("drive.google.com/file/d/") && iframeSrc.includes("/view")) {
                  iframeSrc = iframeSrc.split("/view")[0] + "/preview";
                }
                const ext = sentFilePreviewModal.name.split(".").pop()?.toLowerCase() ?? "";
                const isImage = ["jpg", "jpeg", "png", "gif", "webp", "svg"].includes(ext);

                if (isImage) {
                  return (
                    <img
                      src={sentFilePreviewModal.url}
                      alt={sentFilePreviewModal.name}
                ...
                return (
                  <iframe
                    ...
                    title={sentFilePreviewModal.name}
                    allow="autoplay"
                  />
                );
              })()}
