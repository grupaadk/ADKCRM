...
export default function OrderDriveFilePicker({
  ...
  return (
    <div className="flex flex-col gap-2 flex-1 h-full min-h-0">
      <div className="flex items-center justify-between shrink-0">
        ...
        {selectedList.length > 0 && (
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300">
            Zaznaczono {selectedList.length} {selectedList.length === 1 ? "plik" : "plików"}
          </span>
        )}
      </div>

      {/* Nawigacja okruszkowa (Breadcrumb) */}
      <div className="flex items-center gap-1 text-xs bg-slate-100 p-2 rounded-md border border-slate-200 overflow-x-auto shrink-0">
      ...
      {loading ? (
        <div className="text-xs text-slate-500 py-12 italic flex flex-1 items-center justify-center gap-2 bg-slate-50 rounded-lg border border-slate-200 min-h-0">
        ...
        <div className="text-xs text-rose-600 p-4 flex-1 bg-rose-50 rounded-lg border border-rose-200 min-h-0">
          {fetchError}
        </div>
      ) : items.length === 0 ? (
        <div className="text-xs text-slate-400 p-4 italic flex-1 bg-slate-50 rounded-lg border border-slate-200 text-center min-h-0 flex items-center justify-center">
          Ten folder jest pusty.
        </div>
      ) : (
        <div className="flex flex-col gap-1 flex-1 min-h-0 overflow-y-auto p-1.5 bg-slate-50 border border-slate-200 rounded-lg">
          {items.map((item) => {
            if (item.isFolder) {
              return (
                <div
